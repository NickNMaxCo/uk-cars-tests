Feature: Find car
  As a user, I want to use search filters, so I can have quick and easy way to find a car that suits me.

  Background:
    Given search page is opened

  Scenario: Checking correct postcode
    When user fill in Postcode field with E203EL
    And click the Search cars button
    Then on the page with found cars a Make filter is selected as Any
  ...
  Scenario: Checking incorrect postcode
    When user fill in Postcode field with E203E1
    And click the Search cars button
  ...
  Scenario Outline: Checking 'Make', 'Fuel consumption', 'Doors', 'Annual tax', 'Acceleration', 'Colour', 'Fuel type',
    'Gearbox', 'Private & trade', 'Insurance Group' filters
    When user fill in Postcode field with E203EL
  ...
  Scenario: Checking 'Reset search' link
    When user sets some filters
    And click the Reset search link
    Then all filters are reseted

  Scenario Outline: Checking dependence between 'Min price' and 'Max price' filters
    When user fill in Postcode field with E203EL
  ...
  Scenario Outline: Checking dependence between 'Min seats' and 'Max seats' filters
    When user fill in Postcode field with E203EL
  ...
  Scenario Outline: Checking 'CO2 emissions' filter
    ...
    And select <filter_value_option> from <filter_name> filter
    And click the Search cars button
    Then all cars with <filter_option> or less <filter_name> are shown
